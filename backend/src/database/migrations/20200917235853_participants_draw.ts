...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable("participants_draw", function (table) {
    ...
    table.integer("draw_quotas_draw_quota_id").notNullable().references("draw_quotas.draw_quota_id");
   // table.foreign("draw_quotas_draw_quota_id").references("draw_quotas.draw_quota_id");
    table.integer("users_user_id").notNullable().references("users.user_id");;

    //table.foreign("users_user_id").references("users.user_id");
    table.integer("draws_draw_id").notNullable().references("draws.draw_id");
